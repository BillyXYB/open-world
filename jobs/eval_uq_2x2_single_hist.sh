#!/bin/bash
#
#SBATCH --job-name=eval-uq-2x2-single-hist
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
# ===== CHECKPOINT (auto-select latest from UQ 2x2 single-hist training run) =====
CKPT_DIR="checkpoints/wm_libero/libero_wm_uq_2x2_single_hist_v0"
...
uv run python scripts/plot_uq_by_cell.py \
    --csv "${RESULTS_DIR}/uq_per_episode.csv" \
