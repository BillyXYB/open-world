#!/bin/bash
#
#SBATCH --job-name=replay-droid-epi-fo
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
# ===== CHECKPOINT =====
# Trained by jobs/train_wm_uq_flow_matching_4_gpu_future_overlap_droid.sh
# (configs/training/droid_wm_uq_future_overlap.py, p_history_future_overlap=0.5).
# v1 (history_overlap_noise_scale=0.3): v0 (0.05) showed the logvar head
# collapsing to near-uniform overconfidence on overlap slots -- see
# droid_wm_uq_future_overlap.py's v1 docstring. v0's checkpoints/replay
# remain on disk under droid_flow_matching_uq_future_overlap_v0/ for comparison.
...
# input is in-distribution (unlike feeding this mode to a future_hist/vanilla
# checkpoint, which was never exposed to this conditioning at train time).
...
uv run scripts/replay_libero_wm_traj.py \
    ...
    --output_dir "${CKPT_DIR}/replay_epi_future_overlap" \
