...
class UNetSpatioTemporalConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        # add distance conditioning, as in Flowmap. 
        self.distance_conditioning = distance_conditioning
        if self.distance_conditioning:
            self.distance_proj = Timesteps(block_out_channels[0], True, downscale_freq_shift=0)
            self.distance_embedding = TimestepEmbedding(timestep_input_dim, time_embed_dim)
            
            # IMPORTANT: zero-init so pretrained behavior is preserved initially
            nn.init.zeros_(self.distance_embedding.linear_1.weight)
            nn.init.zeros_(self.distance_embedding.linear_1.bias)
            nn.init.zeros_(self.distance_embedding.linear_2.weight)
            nn.init.zeros_(self.distance_embedding.linear_2.bias)
    ...
    def forward(
        ...
        if self.distance_conditioning:
            if distance is None:
                raise ValueError("Distance conditioning is enabled, but no distance tensor is provided.")
            distance_emb = self.distance_proj(distance.flatten())
            distance_emb = distance_emb.to(dtype=sample.dtype)
            distance_emb = self.distance_embedding(distance_emb)
            emb = emb + distance_emb
